...
async def check_single_account(cookie: str, original_line: str = "", user_id: int = None) -> dict:
    ...
    settings = get_user_settings(user_id) if user_id else {'exact_brainrot_check': True}
...
async def process_checker(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        for account in account_data:
            ...
            account_info = await check_single_account(account['cookie'], account['original_line'], callback.from_user.id)
...
# Добавьте остальные обработчики (fresher и другие) из предыдущей версии
...
async def main():
    try:
        ...
        logger.info("🌌 Запускаю Celestial Checker...")
        me = await bot.get_me()
